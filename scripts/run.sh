...
qemu-system-x86_64 \
	...
	-net nic,model=virtio						`#using virtio drivers for networking` \
	-soundhw ac97							`#using realtek ac97 driver for guest audio` \
	-device vfio-pci,host=01:00.0					`#attaching the respective PCI devices` \
	...
	`#using virtio drivers instead of IDE to improve the performance. l2_cache_size = disk_size * 8 / cluster_size. The default cluster size is 64K` \
